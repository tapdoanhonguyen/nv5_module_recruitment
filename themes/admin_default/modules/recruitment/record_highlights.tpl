		...
		<table class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th class="w150">{LANG.record_code}</th>
					<th>{LANG.title}</th>
					<th class="w100 text-center">{LANG.is_hot}</th>
					<th class="w100 text-center">{LANG.is_highlights}</th>
					<th class="w100">{LANG.time_begin}</th>
					<th class="w100">{LANG.time_end}</th>
...
<script>
	function nv_change_status(id) {
		...
		if (confirm(nv_is_change_act_confirm[0])) {
			...
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=record_highlights&nocache=' + new Date().getTime(), 'change_status=1&id=' + id, function(res) {
