...
<!-- BEGIN: post_new -->
<h1 class="wtitle pull-left">{LANG.hot_post}</h1>
<div class="form-inline text-center pull-right">
	<select id="jobs" class="form-control input-sm" onchange="nv_get_post_hot_condition()">
		<option value="0">{LANG.jobs_a}</option>
		<!-- BEGIN: jobs -->
		<option value="{JOBS.id}">{JOBS.title}</option>
		<!-- END: jobs -->
	</select>
	<select id="per_page" class="form-control input-sm" onchange="nv_get_post_hot_condition()">
		<!-- BEGIN: per_page -->
