...
if( ! nv_function_exists( 'nv_block_recruitment_search' ) )
{
	function nv_block_config_recruitment_search( $module, $data_block, $lang_block )
	{
		global $site_mods;
	...
	function nv_block_config_recruitment_search_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array( );
		$return['error'] = array( );
	...
	function nv_block_recruitment_search( $block_config )
	{
		global $nv_Request, $module_info, $site_mods, $module_name, $lang_module, $module_config, $global_config, $array_jobs, $array_province, $array_worktype, $array_recruitment_position, $array_learning, $db, $db_config;
		...
		if( $block_config['search_template'] == 'vertical' )
		{
			$block_name = 'block_search_vertical.tpl';
		}
		else
		{
			$block_name = 'block_search_horizontal.tpl';
		}

		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme']  . '/modules/' . $mod_file . '/' . $block_name ) )
		{
		...
		$xtpl = new XTemplate( $block_name, NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file );
		...
		*/
		...
		{
			if($tinhthanh['provinceid'] == $array_search['provinceid'])
			{
		...
		{
			$list_quan = $db->query('SELECT districtid, title, type FROM ' . $db_config['prefix'] . '_location_district WHERE provinceid = '. $array_search['provinceid'] .' and status = 1 ORDER BY weight DESC')->fetchAll();
		
			foreach($list_quan as $tinhthanh)
			{	
				if($tinhthanh['districtid'] == $array_search['districtid'])
				{
				$tinhthanh['selected'] = 'selected=selected';
				}
				else $tinhthanh['selected'] = '';
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	$content = nv_block_recruitment_search( $block_config );
}