...
$row['record_id'] = $nv_Request->get_int( 'record_id', 'post,get', 0 );
$is_modal = $nv_Request->get_int( 'is_modal', 'post,get', 0 );

if( $row['id'] > 0 )
{
	$row = $db->query( 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_record_highlights WHERE id=' . $row['id'] )->fetch();
	if( empty( $row ) )
	{
...
list( $row['title'], $row['code'] ) = $db->query( 'SELECT title, code FROM ' . NV_PREFIXLANG . '_' . $module_data . '_record WHERE id=' . $row['record_id'] )->fetch( 3 );

if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	$row['record_id'] = $nv_Request->get_int( 'record_id', 'post', 0 );
	...
	if( empty( $error ) )
	{
		try
		{
			if( empty( $row['id'] ) )
			{
				$stmt = $db->prepare( 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_record_highlights (record_id, is_hot, is_hot_icon, is_highlights, time_begin, time_end, add_time, add_userid, status) VALUES (:record_id, :is_hot, :is_hot_icon, :is_highlights, :time_begin, :time_end, ' . NV_CURRENTTIME . ', :add_userid, 1)' );
				$stmt->bindParam( ':add_userid', $admin_info['userid'], PDO::PARAM_INT );
			}
			else
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_record_highlights SET record_id = :record_id, is_hot = :is_hot, is_hot_icon = :is_hot_icon, is_highlights = :is_highlights, time_begin = :time_begin, time_end = :time_end WHERE id=' . $row['id'] );
			}
